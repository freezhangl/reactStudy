<template>
  <div id="yycenter__bottom2" style="width:100%;height:100%;"></div>
</template>
...
export default {
  ...
  watch: {
    rightCenterInfoNew: {
      handler(val) {
        ...
        this.$nextTick(() => {
          if (this.myChartPieRight2) {
            this.myChartPieRight2.clear()
            this.myChartPieRight2.setOption(this.option)
          }
        })
      },
      immediate: true,
        ...
        series: [
          {
            ...
            itemStyle: {
              ...
              normal: {
                color: new this.$echarts.graphic.LinearGradient(0, 0, 0, 1, [
                  { offset: 0, color: 'rgba(25, 217, 255, 1)' },
                  { offset: 1, color: 'rgba(25, 217, 255, 0.1)' }
                ])
                // barBorderRadius: 5,
  ...
  methods: {
    ...
    drawPie() {
      this.myChartPieRight2 = this.$echarts.init(document.getElementById('yycenter__bottom2'))
      // 响应式变化
      window.addEventListener('resize', () => this.myChartPieRight2.resize(), false)
    }
  },
  destroyed() {
